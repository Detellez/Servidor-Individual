...
(function() {
    ...
    function getIDCorrecto() {
        ...
        // 🔥 FIX: Limpiamos todo texto extra (como una 'p' pre-existente) para no duplicar a 'pp'
        const soloNumeros = valor.replace(/[^0-9]/g, '');
        return isVarious ? soloNumeros : 'p' + soloNumeros;
    }
...
// 🔥 FUNCIÓN HÍBRIDA "BULLDOG": BUSCA, ESPERA Y COMPARTE INFORMACIÓN
   ...
   function renderizarMejorasNativas() {
        ...
        document.querySelectorAll('div.mb-10').forEach(div => {
            ...
            // --- B. CORREO ELECTRÓNICO (Burbuja Dinámica con Ráfaga) ---
            ...
            if (esCorreo && !texto.startsWith('APP:')) {
                ...
                if (emailMatch) {
                    // 🔥 FIX: Guardar el correo original la primera vez para no perder la referencia.
                    if (!div.dataset.originalEmail) {
                        div.dataset.originalEmail = emailMatch[0];
                    }
                    
                    let originalEmail = div.dataset.originalEmail;
                    let emailFinal = originalEmail; 
                    let label = texto.split(':')[0].trim();
                    
                    // Colores por defecto (Verde Original)
                    let colorTexto = '#047857';
                    let colorFondo = 'rgba(16, 185, 129, 0.15)';
                    let colorBorde = 'rgba(16, 185, 129, 0.3)';
                    let hoverBg = 'rgba(16, 185, 129, 0.25)';
                    let tooltipText = "Click para copiar";

                    // Comparativa con LOTE_RAFAGA usando tu ID actual
                    try {
                        const idActual = getIDCorrecto(); 
                        const loteRaw = localStorage.getItem('LOTE_RAFAGA');
                        if (loteRaw && idActual) {
                            const lote = JSON.parse(loteRaw);
                            const clienteRafaga = lote.find(c => c.idPlan === idActual);

                            if (clienteRafaga && clienteRafaga.correo) {
                                const emailRafaga = clienteRafaga.correo.trim();
                                // Comparamos contra el ORIGINAL nativo, no contra el DOM ya modificado
                                if (emailRafaga.toLowerCase() !== originalEmail.toLowerCase()) {
                                    emailFinal = emailRafaga;
                                    colorTexto = '#d97706'; 
                                    colorFondo = 'rgba(245, 158, 11, 0.15)'; 
                                    colorBorde = 'rgba(245, 158, 11, 0.3)';
                                    hoverBg = 'rgba(245, 158, 11, 0.25)';
                                    tooltipText = "⚠️ Editado en Ráfaga. Click para copiar";
                                }
                            }
                        }
                    } catch (e) {
                        console.error("Error al leer LOTE_RAFAGA para el correo:", e);
                    }

                    // Buscamos si el span ya existe
                    let span = div.querySelector('.tellez-email-span');
                    
                    if (!span) {
                        div.innerHTML = `${label}: <span class="tellez-email-span"></span>`;
                        span = div.querySelector('.tellez-email-span');
                        
                        span.addEventListener('mouseenter', function() { this.style.background = this.dataset.hoverBg; });
                        span.addEventListener('mouseleave', function() { this.style.background = this.dataset.defaultBg; });
                        span.addEventListener('click', function() {
                            const valCopiar = this.dataset.emailCopy;
                            navigator.clipboard.writeText(valCopiar).then(() => {
                                this.textContent = '✅ Copiado!';
                                setTimeout(() => { this.textContent = valCopiar; }, 1500);
                            });
                        });
                    }

                    // SIEMPRE actualizamos las propiedades
                    span.textContent = emailFinal;
                    span.title = tooltipText;
                    span.style.color = colorTexto;
                    span.style.background = colorFondo;
                    span.style.border = `1px solid ${colorBorde}`;
                    ...
                    span.dataset.defaultBg = colorFondo;
                    span.dataset.emailCopy = emailFinal;
                }
            }

            // --- C. ID PLAN DE PAGO (Burbuja Verde) E INYECCIÓN DE USER ID ---
            if (texto.startsWith('ID Plan de pago:') || texto.startsWith('ID de orden:') || texto.startsWith('ID do Plano de Pagamento:')) {
                
                // 🔥 NUEVO: INYECTAR "USER ID" ARRIBA DEL ID PLAN DE PAGO
                if (!div.parentNode.querySelector('.tellez-injected-userid')) {
                    const urlMatch = window.location.href.match(/userId=([^&]+)/);
                    if (urlMatch && urlMatch[1]) {
                        const userIdVal = urlMatch[1];
                        
                        // Creamos un nuevo contenedor div igual al original
                        const userDiv = document.createElement('div');
                        userDiv.className = 'mb-10 tellez-injected-userid'; 
                        
                        // 🔥 FIX DE ESPACIADO: Forzamos el salto de línea (margen inferior)
                        userDiv.style.marginBottom = '10px';
                        userDiv.style.display = 'block';

                        userDiv.innerHTML = `User ID: <span class="tellez-userid-span" style="
                            color:#047857; font-weight:bold; cursor:pointer;
                            background:rgba(16, 185, 129, 0.15); border-radius:4px;
                            padding:1px 6px; border:1px solid rgba(16, 185, 129, 0.3);
                            user-select:all; transition:background .2s;
                        " title="Click para copiar">${userIdVal}</span>`;

                        // Lo insertamos justo ANTES del contenedor actual (div)
                        div.parentNode.insertBefore(userDiv, div);

                        // Le damos vida a la burbuja (Hover y Copiado)
                        const spanUid = userDiv.querySelector('.tellez-userid-span');
                        spanUid.addEventListener('mouseenter', () => spanUid.style.background = 'rgba(16, 185, 129, 0.25)');
                        spanUid.addEventListener('mouseleave', () => spanUid.style.background = 'rgba(16, 185, 129, 0.15)');
                        spanUid.addEventListener('click', () => {
                            navigator.clipboard.writeText(userIdVal).then(() => {
                                spanUid.textContent = '✅ Copiado!';
                                setTimeout(() => { spanUid.textContent = userIdVal; }, 1500);
                            });
                        });
                    }
                }

                // --- LÓGICA ORIGINAL DEL ID PLAN DE PAGO ---
...
function renderizarBotonSoporteNativo() {
    ...
    // Encapsulado en un bloque para aislar variables y constantes
    {
        ...
        // 🔥 NUEVO: Activa el redibujado de burbujas al editar en el panel
             if (e.key === 'LOTE_RAFAGA') { renderizarMejorasNativas(); } 
           });
